...
def clean_df(df):
    ...
    df = functions.drop_col_cat(df, 140, 'RiskLevel', 'low risk') #drop 140 low risk samples
...
try:
    functions.write_df(df, r'H:\Datasets', '\clean_MaternalHealthRisk.csv') #save cleaned data to csv fileSkl
